...
def create(_, block_structure, att_type, int_dims, final_dims, window_size, step, k, kk,
           metric, act_pow, cls_pow, reduction='r1', **kwargs):
    if att_type != 'none':
        kwargs.update({'module': AttGCN_Module, 'attention': __attention[att_type]})
    else:
        kwargs.update({'module': ResGCN_Module, 'attention': None})
    return TPGCN(int_dims, final_dims, window_size, step, k, kk, metric, act_pow, cls_pow,
                 **(__structure[block_structure]), **(__reduction[reduction]), **kwargs)
