...
def spd_mean_kracher_flow(X: Tensor, G0: Tensor = None, maxiter: int = 50, dim=0, weights=None, return_dist=False,
                          return_XT=False) -> Union[
    Union[tuple[Tensor, Tensor], Tensor, tuple[Any, Union[Tensor, float, float]], tuple[Any, Any]], Any]:
    if X.shape[dim] == 1:
        if return_dist:
    ...
    if G0 is None:
        ...
        # G = torch.einsum('nab,bj->naj',X, weights)
    else:
        G = G0.clone()
    ...
    while (crit > EPS[X.dtype]) and (i < maxiter) and (nu > EPS[X.dtype]):
        ...
        # GT = torch.einsum('nab,bj->naj', XT, weights)
...
def spd_mean_kracher_flow_weights(X: Tensor, G0: Tensor = None, maxiter: int = 50, dim=0, weight=None, weights=None, return_dist=False,
                          return_XT=False) -> Union[
    Union[tuple[Tensor, Tensor], Tensor, tuple[Any, Union[Tensor, float, float]], tuple[Any, Any]], Any]:
    if X.shape[dim] == 1:
        if return_dist:
    ...
    if weight is None:
        n = X.shape[dim]
        weight = torch.ones((*X.shape[:-2], 1, 1), dtype=X.dtype, device=X.device)
        weight /= n

    if G0 is None:
        G = (X * weight).sum(dim=dim, keepdim=True)
        # G = torch.einsum('nab,bj->naj',X, weights)
    else:
        G = G0.clone()
    ...
    while (crit > EPS[X.dtype]) and (i < maxiter) and (nu > EPS[X.dtype]):
        ...
        XT = sym_logm.apply(Ginvsq @ X @ Ginvsq)
        # GT = (XT * weights).sum(dim=dim, keepdim=True)
        GT = torch.einsum('nmtvcp,nmtvj->nmtjcp', XT, weights)
        G = Gsq @ sym_expm.apply(nu * GT) @ Gsq
